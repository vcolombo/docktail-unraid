...
final class Status
{
     ...
     *
     * @return list<array{ok: bool|null, label: string, detail: string, remedy: string}>
     */
     ...
     *
     * @return array{ok: bool|null, label: string, detail: string, remedy: string}
     */
    private static function nodeTagRow(): array
    {
        ...
        if ( ! file_exists(self::TAILSCALE_BIN)) {
            return ['ok' => null, 'label' => 'Node is tagged', 'detail' => 'Tailscale CLI not available', 'remedy' => $remedy];
        }
        ...
        if ( ! is_array($status) || ! isset($status['Self']) || ! is_array($status['Self'])) {
            return ['ok' => null, 'label' => 'Node is tagged', 'detail' => 'Could not read tailscale status', 'remedy' => $remedy];
        }
     ...
     * DockTail's Funnels.
     *
     * @return array{ok: bool|null, label: string, detail: string, remedy: string}|null
     */
